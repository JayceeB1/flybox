...
| FlyGym / NeuroMechFly 2.x | biomechanical fly simulation | planned `dependency` | Apache-2.0 (verified from upstream `LICENSE`) | Prefer package dependency; do not vendor without adding exact notices/version here. |
| MuJoCo | physics backend used by FlyGym | transitive/planned `dependency` | verify exact installed version before release | Treat as external dependency; preserve upstream notices in distributable bundles when required. |
